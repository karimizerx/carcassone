...
public class CarcassonneGame extends JFrame { // Cette partie se lance dans une fenêtre
    ...
    JPanel panelPlateauView, panelDroit, panelGauche, panelPion; // Des panels utiles
    JButton buttonTourner, buttonDefausser, buttonPiocher, buttonStop, buttonRetourMenu; // Des bouttons utiles
    JButton buttonHaut, buttonDroite, buttonBas, buttonGauche, buttonCentre;
    ...
    /// Constructeurs
    public CarcassonneGame(Joueur[] lj, int nbTuile) { // Constructeur avec une liste de joueurs
        ...
        // Creation du plateau
        plateau = new Plateau(nbTuile, true); // Le nombre de tuiles est 72, mais peut être modifié
    ...
    private JPanel createPanelDroit() { // Crée le panel de droite
        ...
        panelDroit.setPreferredSize(new Dimension(120, this.getHeight()));
        ...
        panelPion = createPanelPion();
        panelPion.setPreferredSize(new Dimension(120, 120));
        panelPion.setVisible(false);;
    ...
    private JPanel createPanelPion(){ // Crée le panel pour placer les pions
                // Initalisation du panel
                panelPion = new JPanel(new GridLayout(5,1));
                panelPion.setPreferredSize(new Dimension(90, 100));
        
                // Initialisation des élements
                buttonHaut = new JButton("Haut");
                buttonHaut.setPreferredSize(new Dimension(85, 60));
                buttonDroite = new JButton("Droit");
                buttonDroite.setPreferredSize(new Dimension(85, 60));
                buttonBas = new JButton("Bas");
                buttonBas.setPreferredSize(new Dimension(85, 60));
                buttonGauche = new JButton("Gauche");
                buttonGauche.setPreferredSize(new Dimension(85, 60));
                buttonCentre = new JButton("Centre");
                buttonCentre.setPreferredSize(new Dimension(85, 60));
        
                // AJout des élements dans le panel
                panelPion.add(buttonHaut);
                panelPion.add(buttonDroite);
                panelPion.add(buttonBas);
                panelPion.add(buttonGauche);
                panelPion.add(buttonCentre);
    ...
    private void placerView(int index) { // Gestion du placement
        ...
        c.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                Boolean placementOk = plateau.placer(tuileAPlacer, xpl, ypl, Liste_Joueurs[joueurDuTour]); // On essaye de placer la tuile
                ...
                if (placementOk) {// Si on peut placer la tuile :
                    if (Liste_Joueurs[joueurDuTour].getPion() > 0) {
                        ...
                        if (clickbtn == JOptionPane.YES_OPTION) { // S'il décide de placer un pion
                            int cnt = JOptionPane.showConfirmDialog(CarcassonneGame.this,"Le placer en haut ?", "Placer un pion", JOptionPane.YES_NO_OPTION);
                            if(cnt == JOptionPane.YES_OPTION){
                                plateau.placerPion(Liste_Joueurs[joueurDuTour], xpl , ypl, ((TuileCarcassonne)tuileAPlacer).getHaut());
                            }
                            else{
                                cnt = JOptionPane.showConfirmDialog(CarcassonneGame.this,"Le placer à droite ?", "Placer un pion", JOptionPane.YES_NO_OPTION);
                                if(cnt == JOptionPane.YES_OPTION){
                                    plateau.placerPion(Liste_Joueurs[joueurDuTour], xpl , ypl, ((TuileCarcassonne)tuileAPlacer).getDroite());
                                }
                                else{
                                    cnt = JOptionPane.showConfirmDialog(CarcassonneGame.this,"Le placer en bas ?", "Placer un pion", JOptionPane.YES_NO_OPTION);
                                    if(cnt == JOptionPane.YES_OPTION){
                                        plateau.placerPion(Liste_Joueurs[joueurDuTour], xpl , ypl, ((TuileCarcassonne)tuileAPlacer).getBas());
                                    }
                                    else{
                                        cnt = JOptionPane.showConfirmDialog(CarcassonneGame.this,"Le placer à gauche ?", "Placer un pion", JOptionPane.YES_NO_OPTION);
                                        if(cnt == JOptionPane.YES_OPTION){
                                            plateau.placerPion(Liste_Joueurs[joueurDuTour], xpl , ypl, ((TuileCarcassonne)tuileAPlacer).getGauche());
                                        }
                                        else{
                                            plateau.placerPion(Liste_Joueurs[joueurDuTour], xpl , ypl, ((TuileCarcassonne)tuileAPlacer).getCentre());
                                        }
                                    }
                                }
                            }
    ...
    private boolean appliqueEffectForAll() { // Applique tous les évenements à toutes les cases
        for (int i = 0; i < plateauView.getLigne() * plateauView.getColonne(); i++) {
            // Coordonnées x,y sélectionnées sur le plateauView
