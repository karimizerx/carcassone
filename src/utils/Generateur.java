...
public class Generateur { // Generateur de tuiles spécial :
    ...
    public static int[] temp1 = new int[3]; // On sauvegarde ici un côté de la tuile précédente
    public static int[] temp2 = new int[3]; // On sauvegarde ici un côté de la tuile précédente
        ...
        if (first) { // Si c'est la première, elle est parfaitement aléatoire
            for (int i = 0; i < 12; i++) {
                ...
                if (i < 3) {
                    ...
                    temp1[i] = nb; // On sauvegarde la côté droit de cette première tuile
                }
                if (i < 6 && i >= 3)
            ...
            for (int i = 0; i < 12; i++) {
                nb = random.nextInt(2);
                if (i < 3) {
                    // Le côté droit est identique au côté droit de la 
                    t.getGauche().getTrio()[2 - i] = temp1[i];
                }
                if (i < 6 && i >= 3)
                    t.getBas().getTrio()[5 - i] = temp2[i - 3]; // Le côté bas est identique au côté bas de la précédente
                if (i < 9 && i >= 6){
                    t.getDroite().getTrio()[i - 6] = nb;
                    temp1[i - 6]=nb;
                }
                if (i < 12 && i >= 9){
                    t.getHaut().getTrio()[i - 9] = nb;
